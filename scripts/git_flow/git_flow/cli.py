...
from .commands.branch import (
    ...
    SupportCommand, BugfixCommand, DevelopCommand, MainCommand,
...
COMMAND_HELP = {
    ...
    'branch': {
        ...
        'main': {'args': ['checkout/pull/push', '--rebase'], 'help': 'Manage main branch'},
...
class SetupCommands:
    ...
    def _register_commands(self):
        ...
        def init(
            ...
            non_interactive: bool = typer.Option(False, help="Non-interactive setup")
        ):
            """
            ...
            Args:
                ...
                non_interactive: Skip interactive configuration
            """
            cmd = InitCommand(self.gitflow)
            config_cmd = ConfigCommand(self.gitflow)
            
            try:
                cmd.init(defaults=defaults, force=force)
                if defaults:
                    config_cmd.init(interactive=False)
                else:
                    config_cmd.init(interactive=not non_interactive)
...
class BranchCommands:
    ...
    def _register_commands(self):
        ...
        def main(
            ...
            """Manage main branch"""
            cmd = MainCommand(self.gitflow)
            self._handle_main_command(cmd, subcommand, rebase)
    ...
    def _handle_main_command(self, cmd, subcommand: str, rebase: bool):
        """Handle main branch specific commands"""
        try:
            if subcommand == "checkout":
                cmd.checkout()
            elif subcommand == "pull":
