...
class SupportCommand(BaseCommand):
    def start(self, version: str, base: str):
        ...
        Args:
            ...
            base: Base commit to start from (must be on master)
        """
        ...
        # Verify base is on master
        master_commits = self.gitflow._git_command(['branch', '--no-color', '--contains', base]).splitlines()
        if not any(b.strip() == self.gitflow.master_branch for b in master_commits):
            raise ValueError(f"Base {base} is not a valid commit on {self.gitflow.master_branch}")
    ...
    def list(self, verbose: bool = False) -> List[Tuple[str, str, str, bool]]:
        ...
        for branch in self.gitflow.get_branches():
            ...
            if verbose:
                base = self.gitflow._git_command(['merge-base', branch, self.gitflow.master_branch]).strip()
                master_sha = self.gitflow._git_command(['rev-parse', self.gitflow.master_branch]).strip()
