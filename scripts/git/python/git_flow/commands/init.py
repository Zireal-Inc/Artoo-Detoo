...
class InitCommand(BaseCommand):
    ...
    def init(self, defaults: bool = False, force: bool = False):
        ...
        if not self.gitflow._git_config_get('gitflow.branch.master') or force:
            master = 'master' if defaults else input('Branch name for production releases [master]: ') or 'master'
        ...
        if not self.gitflow._git_config_get('gitflow.branch.develop') or force:
            develop = 'develop' if defaults else input('Branch name for "next release" development [develop]: ') or 'develop'
